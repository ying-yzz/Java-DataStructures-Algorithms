...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        ...
        if(factorCarga() > FACTOR_DE_CARGA){
            
            //Obtenemos el nuevo array de mayor tamaño, todos inicializados a null
            ListaConPI<EntradaHash<C, V>>[] nuevo;
            nuevo = (ListaConPI<EntradaHash<C, V>>[]) new ListaConPI[siguientePrimo(elArray.length*2)];
            
            //En cada posición del array pone una lista vacía
            for(int i = 0; i<nuevo.length;i++){
                nuevo[i] = new LEGListaConPI<EntradaHash<C,V>>();
            }
            
            //Se va trasladar cada elemento del array antiguo a cubetaVieja
            for(int i = 0; i<elArray.length;i++){
                ListaConPI<EntradaHash<C,V>> cubetaVieja = elArray[i];
                
                //Recorre cada paabra que hay dentro de la cubeta
                for(cubetaVieja.inicio(); !cubetaVieja.esFin(); cubetaVieja.siguiente()){
                    EntradaHash<C,V> entrada = cubetaVieja.recuperar();
                    
                    //Calculamos la nueva dirección ya que el array es más grande
                    int nuevaPos =Math.abs(entrada.clave.hashCode()) % nuevo.length;
                    
                    nuevo[nuevaPos].insertar(entrada);
                }
            }
            
            //elArrya ahora tiene todos los elementos de nuevo
            this.elArray = nuevo;
        }
    ...
    public final double desviacionTipica() {
        //Desviación típica usada para indicar qué tan lleno están las tablas
        double desviacion;
        double suma = 0;
        for(int i = 0; i<elArray.length;i++){
            suma += Math.pow(elArray[i].talla() - factorCarga(),2);
        }
        
        desviacion = Math.sqrt(suma/elArray.length);
    ...
    public final double costeMLocalizar() {
        //Colisiones de la cubeta= L*(L-1)/2, siendo L la longitud de la cubeta
        double totalColisiones = 0;
        
        for(int i = 0; i<elArray.length;i++){
            
            //Obtenemos la longitud de cada cubeta
            double li = elArray[i].talla();
            
            //Aplicamos formula de colisiones
            totalColisiones += li*(li-1)/2;
        }
        
        //Devolver el coste promedio(media colisiones)
        return (totalColisiones/this.talla);
    }
